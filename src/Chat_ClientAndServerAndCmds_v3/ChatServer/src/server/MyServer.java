...
import Chat_ClientAndServerAndCmds_v3.ChatServer.src.server.auth.BaseAuthService;
import Chat_ClientAndServerAndCmds_v3.ChatServer.src.server.handler.ClientHandler;
...
public class MyServer {
    ...
    public void start() throws IOException {
        ...
        ServerApp.getLogFromServer().getFile().info("Сервер запущен MyServer");
        ...
        try {
            ...
            ServerApp.getLogFromServer().getFile().error("Ошибка создания нового подключения MyServer");
            e.printStackTrace();
        } finally {
            serverSocket.close();
        }
    ...
    private void waitAndProcessNewClientConnection() throws IOException {
        ...
        ServerApp.getLogFromServer().getFile().info("Клиент подключился MyServer");
    ...
    public synchronized void subscribe(ClientHandler clientHandler) throws IOException {
        ...
        ServerApp.getLogFromServer().getFile().info("Пользователь добавлен, лист пользователей обновлён MyServer");
    }
    ...
    public synchronized void unSubscribe(ClientHandler clientHandler) throws IOException {
        ...
        List<String> usernames = getAllUsernames();
        broadcastMessage(null, Command.updateUsersListCommand(usernames));
        ServerApp.getLogFromServer().getFile().info("Пользователь удалён, лист пользователей обновлён MyServer");
    }
...
//                System.out.println((char) n);
            ...
            // Но ведь он в цикле while, стало быть будет обновляться каждую итерацию.. Надо учесть.
            // Проблема же в том - что после прогрузки до конца письма проходит несколько секунд, прежде, чем можно
            // начать писать. Иначе история будет вновь и вновь подгружаться. Хотя даже и 100*2 пробелов считал (strCount != 0)
            // Пока вариант такой.
            // Да, и ориентир "два пробела" = "одно письмо", поменял бы, хотя бы, на "[" = "1 письмо"
            ...
            ServerApp.getLogFromServer().getFile().info("История сообщений загружена пользователю MyServer");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
